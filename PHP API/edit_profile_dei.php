...
$sqlProfile = "SELECT * FROM DEI_Officer WHERE DEI_ID='$userID'";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $updateProfileQuery = "UPDATE DEI_Officer SET Universityname=?, Contact=?, Address=? WHERE DEI_ID=?";

    $stmt = mysqli_prepare($conn, $updateProfileQuery);

    //     // Bind the parameters to the prepared statement, using 'b' for binary data
    mysqli_stmt_bind_param($stmt, 'ssss', $univName, $contact, $address, $userID);
    if (mysqli_stmt_execute($stmt)) {
        // Redirect to the profile page after successful update
        header("Location: DeiDashboard.php");
        exit();
    } else {
        echo "hi";
        ...
        <div class="midleft">
            <div class="edit-profile-form">
                ...
                <form method="POST" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>">
                    ...
                    <input type="text" name="universityname" value="<?php echo $recruiter['UniversityName']; ?>" required>
                    <br />
                    <label>Contact:</label>
